...
import com.arthas.service.shell.handlers.term.*;
...
import com.arthas.service.utils.FileUtils;
...
public class TermImpl implements Term {
    ...
    public TermImpl(TtyConnection conn) {
        this(com.arthas.service.shell.term.impl.Helper.loadKeymap(), conn);
    }
    ...
    void checkPending() {
        if (stdinHandler != null && readline.hasEvent()) {
            stdinHandler.handle(io.termd.core.util.Helper.fromCodePoints(readline.nextEvent().buffer().array()));
            checkPending();
        }
    }
    ...
    public TermImpl echo(String text) {
        echo(io.termd.core.util.Helper.toCodePoints(text));
        return this;
    }
    ...
    public void handleEof(Integer key) {
        ...
        if (stdinHandler != null) {
            stdinHandler.handle(io.termd.core.util.Helper.fromCodePoints(new int[]{key}));
        } else {
            echo(key);
